...
class UngDungElGamal(QMainWindow):
    ...
    def tao_khung_sinh_khoa(self, layout_cha):
        ...
        self.o_nhap_q.setToolTip(
            "💡 ĐIỀU KIỆN: q phải là số nguyên tố.")
        ...
        layout.addWidget(self.nut_khoa_ngau_nhien, 0, 2)
        ...
        layout.addWidget(QLabel("YA = a^XA mod q ="), 1, 2)
        self.nhan_ket_qua_ya = QLabel("")
        self.nhan_ket_qua_ya.setStyleSheet("font-weight: bold; color: #D9534F;")
        layout.addWidget(self.nhan_ket_qua_ya, 1, 3)

        layout.addWidget(QLabel("Khóa công khai {q, a, YA}:"), 2, 2)
        self.nhan_khoa_cong_khai = QLabel("")
        self.nhan_khoa_cong_khai.setStyleSheet("font-weight: bold; color: #D9534F;")
        layout.addWidget(self.nhan_khoa_cong_khai, 2, 3)

        layout.addWidget(QLabel("Khóa bí mật {XA}:"), 3, 2)
        self.nhan_khoa_bi_mat = QLabel("")
        self.nhan_khoa_bi_mat.setStyleSheet("font-weight: bold; color: #D9534F;")
    ...
    def tao_giao_dien_ma_hoa(self, cha):
        ...
        self.o_nhap_k_ma_hoa.setToolTip(
            "💡 ĐIỀU KIỆN: Số k là khóa phiên dùng một lần (session key), phải thỏa mãn: 1 < k < q")
    ...
    def tao_giao_dien_chu_ky(self, cha):
        ...
        self.o_nhap_k_chu_ky.setToolTip(
            "💡 ĐIỀU KIỆN: 0 < k < q - 1 \ngcd(k,q-1) = 1 (k và q - 1 - nguyên tố cùng nhau)")
        ...
        self.o_nhap_tep_ky.setToolTip(
            "💡 Hỗ trợ các tệp tài liệu văn bản dạng: .doc, .docx, .json, .txt, .pdf, .xls, .xlsx")
        ...
        self.o_nhap_tep_xac_thuc.setToolTip(
            "💡 Hỗ trợ các tệp tài liệu văn bản dạng: .doc, .docx, .json, .txt, .pdf, .xls, .xlsx")
        ...
        self.nut_thuc_hien_xac_thuc = QPushButton("🛡️ Thực hiện Kiểm tra chữ ký")
        self.nut_thuc_hien_xac_thuc.setObjectName("NutBoGoc")
        self.nut_thuc_hien_xac_thuc.clicked.connect(self.hanh_dong_xac_thuc)
        layout_xt.addWidget(self.nut_thuc_hien_xac_thuc)

        layout.addWidget(khung_xt)

    # ================= 2. TIỆN ÍCH CHUNG & MỞ RỘNG (ĐỌC FILE) =================
    ...
    def tao_khoa_ngau_nhien_ui(self):
        while True:
            q = random.randint(2 ** 61, 2 ** 62 - 1)
            if core.kiem_tra_so_nguyen_to(q): break
        while True:
            a = random.randint(2, q - 1)
            if core.kiem_tra_can_nguyen_thuy(a, q): break
        XA = random.randint(1, q - 2)
        self.o_nhap_q.setText(str(q))
        self.o_nhap_a.setText(str(a))
        self.o_nhap_x.setText(str(XA))
    ...
    def xac_nhan_khoa(self):
        try:
            ...
            self.nhan_ket_qua_ya.setText(str(YA))
            self.nhan_khoa_cong_khai.setText(f"{{{q}, {a}, {YA}}}")
            self.nhan_khoa_bi_mat.setText(f"{{{XA}}}")
    ...
    def tao_k_ma_hoa_ngau_nhien_ui(self):
        ...
        self.o_nhap_k_ma_hoa.setText(str(random.randint(1, self.q - 1)))
    ...
    def xac_nhan_k_ma_hoa(self):
        ...
        self.o_nhap_K_lon_ma_hoa.setText(str(Gia_tri_K))
    ...
    def _hien_thi_loi_tu_luong(self, tieu_de, noi_dung, tac_vu_bi_loi):
        ...
        if tac_vu_bi_loi == "ma_hoa":
            ...
            self.nut_thuc_hien_xac_thuc.setEnabled(True)
    ...
    def tao_k_chu_ky_ngau_nhien_ui(self):
        ...
        while True:
            ...
            if math.gcd(k, self.q - 1) == 1:
                self.o_nhap_k_chu_ky.setText(str(k))
                break
    ...
    def hanh_dong_ky(self):
        ...
        def run_thread():
            try:
                r, s, m = core.ky_tep_tin(self.q, self.a, self.XA, self.k_chu_ky_da_chot, duong_dan)
                self.m_vua_ky = m
    ...
    def hanh_dong_xac_thuc(self):
        ...
        self.nut_thuc_hien_xac_thuc.setEnabled(False)

        def run_thread():
            try:
                data = json.loads(chuoi_ck)
                m_goc = getattr(self, "m_vua_ky", 0)
                trang_thai = core.xac_minh_chu_ky_chi_tiet(
                    self.q, self.a, self.YA, int(data["r"]), int(data["s"]), m_goc, duong_dan
                )
                self.tin_hieu.hoan_thanh_xac_thuc.emit(trang_thai)
            except Exception as e:
                self.tin_hieu.bao_loi.emit("Lỗi", "Chữ ký sai định dạng!", "xac_thuc")
    ...
    def _phan_hoi_xac_thuc_chi_tiet_ui(self, trang_thai):
        ...
        if trang_thai == "HOP_LE":
            ...
            QMessageBox.critical(self, "❌ VĂN BẢN VÀ CHỮ KÝ ĐỀU BỊ SỬA ĐỔI",
                                 "Văn bản không toàn vẹn và chữ ký không khớp")