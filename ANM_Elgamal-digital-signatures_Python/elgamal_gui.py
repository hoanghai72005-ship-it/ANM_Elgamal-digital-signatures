...
class UngDungElGamal:
    ...
    def tao_khung_sinh_khoa(self):
        ...
        # CẬP NHẬT: Thay đổi nội dung các GhiChu (Tooltip) bám sát điều kiện toán học
    ...
    def tao_khoa_ngau_nhien_ui(self):
        while True:
            ...
            if core.la_so_nguyen_to(q): break
        while True:
            a = random.randint(2, q - 1)
            if core.la_can_nguyen_thuy(a, q): break
    ...
    def xac_nhan_khoa(self):
        try:
            ...
            if not core.la_so_nguyen_to(q): raise ValueError("q phải là số nguyên tố!")
            if not core.la_can_nguyen_thuy(a, q): raise ValueError(f"a phải là căn nguyên thủy của {q}!")
    ...
    def xac_nhan_k_ma_hoa(self):
        ...
        self._khoa_o_nhap(self.o_nhap_k_ma_hoa)
        Gia_tri_K = core.tinh_luy_thua_module(self.YA, k, self.q)
    ...
    def hanh_dong_ky(self):
        ...
        def luong_xu_ly():
            try:
                r, s, m_goc = core.ky_tep(self.q, self.a, self.XA, self.k_chu_ky_da_chot, duong_dan_tep)
        ...
        def luong_xu_ly():
            try:
                ...
                if m_goc is not None:
                    trang_thai = core.xac_thuc_chu_ky_chi_tiet(self.q, self.a, self.YA, r, s, int(m_goc), duong_dan_tep)
                    self.goc.after(0, self._phan_hoi_xac_thuc_chi_tiet_ui, trang_thai)
                else:
                    hop_le = core.xac_thuc_chu_ky(self.q, self.a, self.YA, r, s, duong_dan_tep)
